...
{
    public partial class IT_Employee_Info : Form
    {
        MySqlDataAdapter adpt;
        DataTable dt;
        public IT_Employee_Info()
        {
            InitializeComponent();
        ...
        private void IT_Employee_Info_Load(object sender, EventArgs e)
        {
            string constring = "server=localhost;uid=root;pwd=remote1234;database=company";
            MySqlConnection con = new MySqlConnection();
            con.ConnectionString = constring;
            adpt = new MySqlDataAdapter("SELECT employee.EmployeeId, employee.Name, employee.ContactNumber, project.ProjectName FROM employee INNER JOIN project ON employee.Project_idProject = project.idProject", con);
            dt = new DataTable();
            adpt.Fill(dt);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (MySqlConnection con = new MySqlConnection(constring))
            {
                try
                {
                    ...
                    {
                        string query = "Select * From employee Where EmployeeId = @SearchText and employee.Department_idDepartment = 4";
                    ...
                    {
                        string query = "Select * From employee Where Name = @SearchText and employee.Department_idDepartment = 4";
                        adpt = new MySqlDataAdapter(query, con);
                        adpt.SelectCommand.Parameters.AddWithValue("@SearchText", text);
                        adpt.Fill(dt);
                        dataGridView1.DataSource = dt;
